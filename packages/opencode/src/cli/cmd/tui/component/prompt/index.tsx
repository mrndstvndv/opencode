...
export function Prompt(props: PromptProps) {
  ...
  const kv = useKV()
  const zenMode = createMemo(() => kv.get("zen_mode", false))
  ...
  createEffect(() => {
    if (props.disabled) input.cursorColor = zenMode() ? theme.background : theme.backgroundElement
  ...
  return (
    <>
      ...
      <box ref={(r) => (anchor = r)} visible={props.visible !== false}>
        <box
          border={zenMode() ? ["top", "bottom"] : ["left"]}
        ...
        >
          <box
            paddingLeft={zenMode() ? 1 : 2}
            paddingRight={zenMode() ? 1 : 2}
            paddingTop={zenMode() ? 0 : 1}
            flexShrink={0}
            backgroundColor={zenMode() ? theme.background : theme.backgroundElement}
            flexGrow={1}
          >
            <Show when={zenMode()}>
            ...
            <textarea
              ...
              focusedBackgroundColor={zenMode() ? theme.background : theme.backgroundElement}
                ...
                <Show when={store.mode === "normal"}>
                  <box flexDirection="row" gap={1}>
                    ...
                    <Show when={showVariant()}>
                      ...
                      <text>
                        <span style={{ fg: keybind.leader ? theme.textMuted : theme.warning, bold: !keybind.leader }}>
                          {local.model.variant.current()}
                        </span>
                      </text>
                    </Show>
        ...
        <Show when={!zenMode() || status().type === "retry"}>
          <box flexDirection="row" justifyContent="space-between">
            ...
            <Show when={status().type !== "retry" && !zenMode()}>
